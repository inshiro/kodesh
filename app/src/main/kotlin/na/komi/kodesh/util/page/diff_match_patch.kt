...
class diff_match_patch {
     ...
     */
    @JvmOverloads
    fun diff_main(text1: String, text2: CharSequence,
            ...
            when (thisDiff.operation) {
                ...
                diff_match_patch.Operation.EQUAL -> {
                    ...
                    if (count_delete >= 1 && count_insert >= 1) {
                        ...
                        for (subDiff in diff_main(text_delete, text_insert, false,
                                deadline)) {
                            pointer.add(subDiff)
                        }
                    }
    ...
    private fun diff_linesToCharsMunge(text: String, lineArray: MutableList<String>,
                                       lineHash: MutableMap<String, Int>, maxLines: Int): String {
        var lineStart = 0
        var lineEnd = -1
        var line: String
     ...
     * @param lineArray List of unique strings.
     */
    protected fun diff_charsToLines(diffs: List<Diff>,
     ...
     */
    fun diff_commonPrefix(text1: String, text2: CharSequence): Int {
    ...
    fun diff_cleanupSemanticLossless(diffs: LinkedList<Diff>) {
        ...
        while (nextDiff != null) {
            if (prevDiff!!.operation == Operation.EQUAL && nextDiff.operation == Operation.EQUAL) {
                ...
                while (edit!!.isNotEmpty() && equality2!!.isNotEmpty()
                        && edit[0] == equality2[0]) {
                    equality1 += edit[0]
        ...
        while (thisDiff != null) {
            if (thisDiff.operation == Operation.EQUAL) {
                ...
                if (lastEquality != null && (pre_ins && pre_del && post_ins && post_del || lastEquality.length < Diff_EditCost / 2 && ((if (pre_ins) 1 else 0) + (if (pre_del) 1 else 0)
                    ...
                    pointer.add(
                            {
                                thisDiff = Diff(Operation.INSERT, lastEquality)
                                thisDiff
                            }()!!
                    )
        ...
        for (token in tokens) {
            ...
            when (token[0]) {
                ...
                '=' -> {
                    ...
                    val text: String
                    try {
                        text = text1.substring(pointer, { pointer += n; pointer }())
